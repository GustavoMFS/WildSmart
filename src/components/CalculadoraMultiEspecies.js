...
const CalculadoraMultiEspecies = (props) => {
    ...
    const calcular = () => {
        const res = [];
        if (props.medIndefinida && props.medIndefinida.length > 0) {
            props.medIndefinida.forEach((medicamento, index) => {
                const { nome, especies, doses, concentracao, unidade } = medicamento;

                especies.forEach((especie, especieIndex) => {
                    const doseMinima = doses[especieIndex][0];
                    const doseMaxima = doses[especieIndex][1];
                    const concentracaoSelecionada = concentracoesSelecionadas[index] || concentracao[0];
                    ...
                    res.push({
                        ...
                        concentracaoSelecionada: concentracaoSelecionada,
                        unidade: unidade,
                        medicamentoIndex: index,
                    });
                });
            });
        }
    ...
    const renderTableData = () => {
        return resultados.map((res, index) => {
            const { nomeMedicamento, especie, doseMinima, doseMaxima, volumeMinimo, volumeMaximo, concentracaoSelecionada, unidade, medicamentoIndex } = res;
            const maxLength = Math.max(...props.medIndefinida[medicamentoIndex].concentracao.map(conc => `${conc} ${unidade}`.length));
                ...
                <View key={index} style={styles.linha}>
                    ...
                    <View style={{ position: 'relative', flex: 2 }}>
                        ...
                        <Picker
                            ...
                            style={{ position: 'absolute', width: pickerWidth, height: 50, opacity: 0 }}
                            onValueChange={(itemValue) => handleConcentracaoChange(medicamentoIndex, itemValue)}>
                            {props.medIndefinida[medicamentoIndex].concentracao.map((concentracao, i) => (
