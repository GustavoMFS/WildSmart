...
const DesgasteCoelho = ({ navigation }) => {
    ...
    const calcular = () => {
        const res = [];
        for (let i = 0; i < AnestesiaCoelhoDesgaste.length; i++) {
            const nomeMedicamento = AnestesiaCoelhoDesgaste[i].nome;
            const doses = AnestesiaCoelhoDesgaste[i].doses;
            const concentracao = AnestesiaCoelhoDesgaste[i].concentracao;
